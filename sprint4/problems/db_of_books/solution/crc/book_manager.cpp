...
bool BookManager::add_book(const std::string& title, const std::string& author, 
    ...
    try {
        ...
        // Подготавливаем statements если они еще не подготовлены
        if (!conn.is_prepared("insert_book_with_isbn")) {
            conn.prepare("insert_book_with_isbn",
                "INSERT INTO books (title, author, year, isbn) VALUES ($1, $2, $3, $4)");
        }
        if (!conn.is_prepared("insert_book_null_isbn")) {
            conn.prepare("insert_book_null_isbn",
                "INSERT INTO books (title, author, year, isbn) VALUES ($1, $2, $3, NULL)");
        }
        
        if (isbn.empty() || isbn == "null") {
            // Insert with NULL ISBN
            txn.exec_prepared("insert_book_null_isbn", title, author, year);
        } else {
            // Insert with provided ISBN
            txn.exec_prepared("insert_book_with_isbn", title, author, year, isbn);
        }
...
std::vector<Book> BookManager::get_all_books() {
    ...
    try {
        ...
        auto result = txn.exec_prepared("get_all_books");
