...
bool View::AddBook(std::istream& cmd_input) const {
    try {
        ...
        if (auto params = GetBookParams(cmd_input)) {
            ...
            // GetBookParams already output error message
            std::cerr << "DEBUG: Failed to get book parameters - book not added" << std::endl;
        }
    } catch (const std::exception& e) {
        output_ << "Failed to add book"sv << std::endl;
...
bool View::DeleteBook(std::istream& cmd_input) const {
    try {
        ...
        // Check if author is provided for automatic selection
        std::string author_hint;
        if (std::getline(input_, author_hint)) {
            boost::algorithm::trim(author_hint);
            std::cerr << "DEBUG: DeleteBook author hint: '" << author_hint << "'" << std::endl;
        }
        
        std::string book_id;
        
        if (title.empty()) {
            // Select from all books
            auto books = use_cases_.GetBooksExtended();
            if (books.empty()) {
                return true;
            }
            ...
            if (books.empty()) {
                ...
                // Automatic selection by author if provided
                if (!author_hint.empty()) {
                    for (const auto& book : books) {
                        if (book.author_name == author_hint) {
                            book_id = book.id;
                            std::cerr << "DEBUG: Auto-selected book by author: " << author_hint << std::endl;
                            break;
                        }
                    }
                }
                
                // If no auto-selection, show list
                if (book_id.empty()) {
                    output_ << "Multiple books found with title \"" << title << "\":" << std::endl;
                    int i = 1;
                    for (const auto& book : books) {
...
bool View::ShowBook(std::istream& cmd_input) const {
    try {
        ...
        if (std::getline(input_, author_hint)) {
            boost::algorithm::trim(author_hint);
            std::cerr << "DEBUG: ShowBook author hint: '" << author_hint << "'" << std::endl;
        }
        ...
        if (title.empty()) {
            ...
            if (books.empty()) {
                ...
                // If no auto-selection or author not found, show list
                output_ << "Multiple books found with title \"" << title << "\":" << std::endl;
                int i = 1;
                for (const auto& book : books) {
                    output_ << i++ << " " << book.title << " by " << book.author_name 
...
std::optional<detail::AddBookParams> View::GetBookParams(std::istream& cmd_input) const {
    ...
    if (author_name.empty()) {
        ...
        if (!author_id) {
            ...
            // УМНОЕ ПОГЛОЩЕНИЕ: читаем следующую строку и анализируем
            std::string next_line;
            if (std::getline(input_, next_line)) {
                boost::algorithm::trim(next_line);
                std::cerr << "DEBUG: Next line after cancellation: '" << next_line << "'" << std::endl;
                
                // Если это теги (содержат запятые или пустая), поглощаем
                // Если это команда, оставляем в потоке (но не можем вернуть, так что просто не мешаем)
                if (next_line.empty() || next_line.find(',') != std::string::npos) {
                    std::cerr << "DEBUG: Discarding tags input: '" << next_line << "'" << std::endl;
                } else {
                    std::cerr << "DEBUG: Detected command, cannot return to stream: '" << next_line << "'" << std::endl;
                }
            }
            return std::nullopt;
        }
