...
bool View::ShowBook(std::istream& cmd_input) const {
    try {
        ...
        if (title.empty()) {
            // Показываем список всех книг для выбора
            auto books = use_cases_.GetBooksExtended();
            if (books.empty()) {
                return true;
            }
            ...
            if (choice.empty()) {
                return true;  // Отмена - возвращаем true, но ничего не выводим
            }
            ...
            try {
                ...
                // Неправильный ввод - просто выходим
            }
            ...
            if (books.empty()) {
                // Книга не найдена - ничего не выводим (по требованию задания)
                return true;
            } else if (books.size() == 1) {
                // Одна книга - показываем детали
                PrintBookDetails(books[0]);
            } else {
                // Несколько книг - предлагаем выбрать
...
bool View::EditBook(std::istream& cmd_input) const {
    try {
        ...
        // Очистка буфера перед вводом данных
        input_.clear();
        ...
        // Очистка буфера перед вводом года
        input_.clear();
        ...
        // Очистка буфера перед вводом тегов
        input_.clear();
...
std::optional<detail::AddBookParams> View::GetBookParams(std::istream& cmd_input) const {
    ...
    // Читаем всю строку и разбираем ее
    std::string line;
    if (!std::getline(cmd_input, line)) {
        std::cerr << "DEBUG: Failed to read input line" << std::endl;
        return std::nullopt;
    }
    
    std::istringstream line_stream(line);
    
    if (!(line_stream >> params.publication_year)) {
        std::cerr << "DEBUG: Failed to read publication year from: '" << line << "'" << std::endl;
        return std::nullopt;
    }

    // Читаем оставшуюся часть как название
    std::string title;
    ...
    // Очистка буфера перед запросом автора
    input_.clear();
    ...
    if (author_name.empty()) {
        ...
        std::cerr << "DEBUG: GetAuthorByName result: " << (author ? "FOUND" : "NOT FOUND") << std::endl;
if (author) {
    std::cerr << "DEBUG: Found author - id: " << author->id << ", name: " << author->name << std::endl;
}
        if (!author) {
    ...
    // Очистка буфера перед запросом тегов
    input_.clear();
