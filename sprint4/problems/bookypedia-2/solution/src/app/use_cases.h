...
namespace app {
    ...
    public:
        ...
        virtual void DeleteAuthor(const std::string& author_id) = 0;  // ¬озвращаем оригинальную сигнатуру
        virtual void EditAuthor(const std::string& author_id, const std::string& new_name) = 0;
        virtual void DeleteBook(const std::string& book_id) = 0;  // ¬озвращаем оригинальную сигнатуру
        ...
        virtual std::vector<BookInfoExtended> GetBooksByTitle(const std::string& title) = 0;
        virtual std::optional<BookInfoExtended> GetBookById(const std::string& book_id) = 0;
