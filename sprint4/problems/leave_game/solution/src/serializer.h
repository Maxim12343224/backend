...
namespace serializer {
    ...
    // Сериализация временных меток
    json::value SerializeTimePoint(const std::chrono::steady_clock::time_point& tp) {
        auto duration = tp.time_since_epoch();
        auto millis = std::chrono::duration_cast<std::chrono::milliseconds>(duration).count();
        return json::value(static_cast<int64_t>(millis));
    }

    std::chrono::steady_clock::time_point DeserializeTimePoint(const json::value& tp_val) {
        auto millis = tp_val.as_int64();
    ...
    json::value SerializePlayer(const SerPlayer& player) {
        ...
        return {
            ...
            {"bag", bag_json},
            {"join_time", SerializeTimePoint(player.join_time)}
        };
    }
    ...
    SerPlayer DeserializePlayer(const json::value& player_val) {
        ...
        return {
            ...
            bag,
            DeserializeTimePoint(obj.at("join_time"))
        };
    }
    ...
    json::value SerializeSession(const SerGameSession& session) {
        ...
        return {
            ...
            {"retirement_time", session.retirement_time},
    ...
    SerGameSession DeserializeSession(const json::value& session_val) {
        ...
        return {
            ...
            obj.at("retirement_time").as_double(),
    ...
    json::value GameSerializer::SerializeGame(const model::Game& game) {
        ...
        state["retirement_time"] = game.GetRetirementTime();
    ...
    void GameSerializer::DeserializeGame(model::Game& game, const json::value& data) {
        ...
        try {
            ...
            if (obj.contains("retirement_time")) {
                game.SetRetirementTime(obj.at("retirement_time").as_double());
                std::cout << "DEBUG: Set retirement_time: " << obj.at("retirement_time").as_double() << std::endl;
            }
            ...
            if (obj.contains("sessions")) {
                ...
                for (size_t session_idx = 0; session_idx < sessions_arr.size(); ++session_idx) {
                    ...
                    for (size_t player_idx = 0; player_idx < ser_session.players.size(); ++player_idx) {
                        ...
                        player->SetJoinTime(ser_player.join_time);
