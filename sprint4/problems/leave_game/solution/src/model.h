...
namespace model {
    ...
    private:
        ...
        void AddToMemory(const std::string& name, int score, double play_time) {
            ...
            // Сортируем при каждом добавлении для простоты
            std::sort(memory_records_.begin(), memory_records_.end(),
                [](const auto& a, const auto& b) {
                    if (std::get<1>(a) != std::get<1>(b))
                        return std::get<1>(a) > std::get<1>(b); // по score убыванию
                    if (std::get<2>(a) != std::get<2>(b))
                        return std::get<2>(a) < std::get<2>(b); // по play_time возрастанию
                    return std::get<0>(a) < std::get<0>(b);     // по name алфавиту
                });
        }
